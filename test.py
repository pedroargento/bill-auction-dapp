...
class BidsTest(unittest.TestCase):
    ...
    def test_valid_bids(self):
        ...
        filtered_bids = list(
            filter_bids(
                ...
                self.auction.id,
                self.auction.end_time,
                self.auction.reserve_price,
            )
        )
        self.assertEqual(
            filtered_bids, expected_filtered_bids, "wrong list of filtered bids"
        )
    ...
    def test_generate_bid_vouchers_mint(self):
        ...
        expected = [
            ...
            Voucher(
                Address("mine_contract"), FunctionCall.MINT, Address("alice"), 30, True
            ),
        ]
        self.assertEqual(list(generate_bid_vouchers(output, price)), expected)
